                ...
                for vlink in K[s][k].edges:
                    ...
                    if var and var.varValue == 1:
                        if (s, k) not in edge_mapping:
                            edge_mapping[(s, k)] = {}
                        # Nếu vlink chưa tồn tại trong edge_mapping[(s, k)], tạo danh sách cho nó
                        if vlink not in edge_mapping[(s, k)]:
                            edge_mapping[(s, k)][vlink] = []
                        # Thêm phy_link vào danh sách các liên kết vật lý của vlink
                        edge_mapping[(s, k)][vlink].append(phy_link)
                        break
    # print(edge_mapping)
