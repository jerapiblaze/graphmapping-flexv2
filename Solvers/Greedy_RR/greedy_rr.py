...
from Solvers.Greedy_RR.ultilities import *
from utilities.profiler import StopWatch

def GreedyRR(PHY:nx.DiGraph, SLICE_SET:list[nx.DiGraph], profiler:StopWatch, timelimit:int=0) -> dict[str, int]:
    ...
    for sfc in SLICE_SET:
        ...
        k = s%len(configurations)
        config = configurations[k]
        _phy, nodeMap, linkMap, info = mapSliceRR(phy, config)
